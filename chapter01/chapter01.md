...
| 검증 항목 | 확인 내용 |
| --- | --- |
| 연령대(10대~60대 이상)별 총 구매 금액, 매출 비중(%), 구매 고객 수, 1인당 평균 구매액 비교 분석 |  |
| customers (고객 정보), orders (주문 정보), order_items (주문 상세) |  |
| customers.age, customers.customer_id, orders.order_id, orders.order_status, order_items.quantity, order_items.unit_price |  |
| order_status == 'completed'인 주문 건 대상 (quantity × unit_price 합산) |  |
|확인 완료 (2025년 7월 ~ 2026년 7월 데이터 기준 총 148,990,000원 집계 완료)|  |
|미단정 (특정 연령대 구매액 차이의 내부/외부 원인을 단정하지 않고, 데이터상의 실적/비중 수치만 비교 제시)|  |
|수정 후 사용|

### 내가 수정한 내용

```text
일정 기준이 상이하여 근 3년간의 매출액 데이터를 살펴보면 여성은 20, 50대가 남성은 50, 60대가 가장 많은 매출액의 대부분을 차지합니다. 기간에 대해 명확한 세팅이 필요하다는 것을 느꼈습니다.
```

### 결과 관찰
명확한 가이드라인이 없으면 결과가 상이해짐을 알 수 있었습니다.

### 나의 해석과 판단
수정 후 사용
- 일정을 명확히 타겟팅 해주니, 원하는 결과가 도출함을 알 수 있었습니다.
...
<img width="745" height="782" alt="step04_validation" src="https://github.com/user-attachments/assets/d9a98db6-2623-4e14-ae89-74bc4c411b70" />

---

## 5. Prompt Log

- 사용 목적: 프로모션 행사에 대한 기획
- 입력 Prompt 요약: 프로모션 행사 일정, 타겟 대상, 타겟 카테고리, 카드 제휴 등 에 대한 프로모션 플랜
- LLM 답변 요약:
■ 프로모션 플랜 핵심 요약
 1. 일정 (When)
 - 10월 메가 세일 (시즌형): 10/15~10/31 (연중 최고 매출 달성 달)
 - Payday 타임 세일 (정례형): 매월 25일~익월 5일 (매출 75% 집중 구간, 화요일 오전 마케팅 집중)
 2. 타겟 (Who)
 - 2030 세대 (매출 49.6% 점유, 핵심 구매층)
 - 2024년 가입자 (매출 39.9% 점유, 코어 충성 고객)
 - 60대 이상 (매출 21.6% 점유, 고액 결제층)
 3. 카테고리 (What)
 - 3대 주력 카테고리: 스포츠(21.3%) + 전자기기(17.7%) + 생활용품(16.1%) 
 - 맞춤 패키지: 2030용(스포츠+전자기기) / 4060용(전자기기+생활용품)
 4. 결제 혜택 (How to Pay)
 - 주요 결제 수단: 네이버페이(29.2%) & 카카오페이(26.4%)
 - 혜택: 30만 원 이상 결제 시 15,000원 즉시 할인/적립 + 신용카드 6개월 무이자 할부(60대 이상 대상)
  
- 실제 반영 여부: 일정에 대한 부분 확인 후 재산정
- 사람이 검증한 항목: 데이터와의 정합성
- 사람이 수정한 내용: 일정에 대한 타겟 명료화
- 남은 확인 사항:
...
<img width="850" height="745" alt="step05_prompt_log" src="https://github.com/user-attachments/assets/6817d3fa-8b51-4d5d-8f38-1faa0542932d" />
